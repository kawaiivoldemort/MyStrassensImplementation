...
int** multiply_strassens_noparallel_nooffset(int** a, int** b, int di, int dj, int dk) {
    int d;
    int** ar, ** br;
    d = di > dj ? di : dj;
    d = d > dk ? d : dk;
    ...
    ar = squarify(a, di, dk, d);
    br = squarify(b, dk, dj, d);
    int** t = multiply_strassens_square_noparallel(ar, 0, 0, br, 0, 0, d);
    free_mat(ar, d);
    free_mat(br, d);
    int** r = compactify(t, di, dj);
    free_mat(t, d);
...
int** compute_c(int*** m, int d) {
    ...
    for(i = 0; i < d; i++) {
        ...
        int j, k;
        int* ct = (int*) MALLOC_A(d2 * sizeof(int));
        int* m0i = m0[i];
        int* m2i = m2[i];
        int* m3i = m3[i];
        int* m4i = m4[i];
